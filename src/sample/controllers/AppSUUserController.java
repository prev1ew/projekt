...
/**
 * @author Vitalii
 * Możliwość dodawania, modyfikowania, usuwania, akceptowania i odrzucania po stronie lekarza
 */
...
//appSUUserAcceptButton
        appSUUserAcceptButton.setOnAction(event -> {
            OrderDetails orderDet = (OrderDetails) appSUUserTable.getSelectionModel().getSelectedItem();
            choosedOrder = orderDet.getIDorders();
            String accept = ("UPDATE " + Const.BD + "." + Const.ORDERS_TABLE + " SET " + Const.ORDERS_STATE + "=\"" + "Accepted" + "\" WHERE (" +
                    Const.ORDERS_ID + "=\"" + choosedOrder + "\")");
            dc.sendCommandToSQL(accept);
            Connection conn = null;
            try {
                conn = dc.getDbConnection();
            } catch (ClassNotFoundException e) {
...
//appSUUserDeclineButton
        appSUUserDeclineButton.setOnAction(event -> {
            OrderDetails orderDet = (OrderDetails) appSUUserTable.getSelectionModel().getSelectedItem();
            choosedOrder = orderDet.getIDorders();
            String decline = ("UPDATE " + Const.BD + "." + Const.ORDERS_TABLE + " SET " + Const.ORDERS_STATE + "=\"" + "Declined" + "\" WHERE (" +
                    Const.ORDERS_ID + "=\"" + choosedOrder + "\")");
            dc.sendCommandToSQL(decline);
            Connection conn = null;
            try {
                conn = dc.getDbConnection();
            } catch (ClassNotFoundException e) {
